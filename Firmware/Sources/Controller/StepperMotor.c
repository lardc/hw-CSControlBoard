...
static volatile Int16U PrevGlobalStepsCounter = 0;
...
ISRCALL Timer1_ISR(void)
{
	...
	if (SM_CycleCounter >= SM_CyclesToToggle) {
		ZbGPIO_ToggleStep();
		SM_CycleCounter = 0;
		if (SM_IsCurrentDirUp) SM_GlobalStepsCounter++;
		else SM_GlobalStepsCounter--;
	}
	else ++SM_CycleCounter;
	...
	if(SM_ChangePositionFlag){
		...
		if (SM_ChangePositionInit) {
			...
			if(StepsToPos > 2 * SM_SPEED_CHANGE_STEPS + SM_LowSpeedSteps + SM_STEPS_RESERVE){
				if (((abs(SM_GlobalStepsCounter - SM_StartSteps) % SM_SpeedChangeDiscrete) == 0) && (PrevGlobalStepsCounter != SM_GlobalStepsCounter)){
					PrevGlobalStepsCounter = SM_GlobalStepsCounter;
					if (SM_CyclesToToggle > SM_MinCycles) SM_UpdCyclesToToggle(--SM_CyclesToToggle);
				}
			}
			...
			// 3. acceleration or deceleration to Vend
				...
				if (((abs(SM_GlobalStepsCounter - SM_StartSteps) % SM_SpeedChangeDiscrete) == 0) && (PrevGlobalStepsCounter != SM_GlobalStepsCounter)){
					PrevGlobalStepsCounter = SM_GlobalStepsCounter;
					if (SM_CyclesToToggle < SM_LowSpeedCycles) SM_UpdCyclesToToggle(++SM_CyclesToToggle);
					else if (SM_CyclesToToggle > SM_LowSpeedCycles) SM_UpdCyclesToToggle(--SM_CyclesToToggle);
				}
			}
			...
			// 5. deceleration to Vmin
				...
				if (((abs(SM_GlobalStepsCounter - SM_StartSteps) % SM_SpeedChangeDiscrete) == 0) && (PrevGlobalStepsCounter != SM_GlobalStepsCounter)){
					PrevGlobalStepsCounter = SM_GlobalStepsCounter;
					if (SM_CyclesToToggle < SM_MaxCycles) SM_UpdCyclesToToggle(++SM_CyclesToToggle);
				}
			}
			if (StepsToPos == 0) {
				if (SM_HomingFlag){
...
Boolean SM_UpdCyclesToToggle(Int16U NewCyclesToTogle){
	if (NewCyclesToTogle <= 1) return FALSE;
	else {
...
Int16U SM_SpeedToCycles(Int16U NewSpeed){
	if (NewSpeed <= SM_MIN_SPEED){
		return (Int16U)(1000000 * SM_MOVING_RER_ROUND / (2 * SM_MIN_SPEED * SM_FULL_ROUND_STEPS * TIMER1_PERIOD));
	}
	else if (NewSpeed < SM_MAX_SPEED){
		return (Int16U)(1000000 * SM_MOVING_RER_ROUND / (2 * NewSpeed * SM_FULL_ROUND_STEPS * TIMER1_PERIOD));
	}
	else return (Int16U)(1000000 * SM_MOVING_RER_ROUND / (2 * SM_MAX_SPEED * SM_FULL_ROUND_STEPS * TIMER1_PERIOD));
}
// ----------------------------------------
