...
// Functions
//
Int16U ZbSU_ReadWrite(Int16U Data)
{
	Int16U InputData = 0;
...
void ZbSU_FIFOStatusUpdate()
{
	Int16U config = ZbSU_ReadWrite(0x4000);
...
void ZbSU_SendData(pInt16U Buffer, Int16U BufferSize)
{
	...
	while (TxCounter < BufferSize)
	{
		do
		{
			ZbSU_FIFOStatusUpdate();
		}
		while(!SU_TxFIFOEmpty);

		ZbSU_ReadWrite((Buffer[TxCounter] & 0xFF) | 0x8000);
		TxCounter++;
	}
